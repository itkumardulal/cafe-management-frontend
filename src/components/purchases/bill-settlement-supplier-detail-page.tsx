...
import { formatPurchasePaymentMethodWithBank, PAYMENT_METHOD_OPTIONS } from "@/src/lib/ap-display";
...
export function BillSettlementSupplierDetailPage() {
  ...
  const [bankAccounts, setBankAccounts] = useState<
    Array<{ id: string; bankName: string; accountNumber: string; label: string }>
  >([]);
  ...
  }, [payMethod, bankAccountId, bankAccounts]);

  const remaining = detail ? Number(detail.supplier.outstandingAmount) : 0;
  const groupedPayments = useMemo(() => {
    if (!detail) return [];

    type PaymentRow = BillSettlementSupplierDetail["paymentHistory"][number];
    type PaymentGroup = {
      receipt: string;
      total: number;
    ...
    for (const payment of detail.paymentHistory) {
      ...
      if (existing) {
        ...
        groups.set(key, {
          ...
          bankAccountLabel: payment.bankAccountLabel ?? null,
    ...
    const totalPurchases = Number(detail.supplier.totalPurchases);
    const chronological = [...groups.values()].sort((a, b) => {
      const dateCmp =
    ...
    for (const group of chronological) {
      cumulativePaid += group.total;
      group.remainingOutstanding = Math.max(0, totalPurchases - cumulativePaid);
    }
  ...
  const bankAccountRequired = payMethod === "BANK_TRANSFER";
  const hasBankAccount = !bankAccountRequired || Boolean(bankAccountId);
  const canSettle =
    remaining > 0.005 &&
  ...
  }, [payAmount, payMethod, payRemarks, bankAccountId]);
                ...
                {groupedPayments.map((g, idx) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-muted">
                      {formatDateTime(g.paymentDate)} ·{" "}
                      {formatPurchasePaymentMethodWithBank({
                        paymentMethod: g.paymentMethod,
                        bankAccountLabel: g.bankAccountLabel,
                      })}
                      {g.rows[0]?.settlementBatchId ? " · FIFO settlement" : ""}
                    </p>
                    <p className="mt-2 border-t border-[var(--color-border)] pt-2 text-xs">
                      ...
                      <span className="font-mono font-semibold tabular-nums tone-warning-text">
                        {formatMoney(g.remainingOutstanding)}
                      </span>
                    </p>
                    {g.rows[0]?.settlementBatchId && g.rows.length > 0 ? (
            ...
            <div className="form-fields">
              ...
              {payMethod === "BANK_TRANSFER" ? (
                <Field
                  ...
                  label="Bank account"
                  required
                  hint="Records a withdrawal in bank transactions for this settlement"
                >
